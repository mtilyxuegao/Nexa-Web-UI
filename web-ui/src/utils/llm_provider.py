...
from .nexa_adapter import create_nexa_llm
...
def get_llm_model(provider: str, **kwargs):
    ...
    :return:
    """
    if provider not in ["ollama", "bedrock", "nexa"]:
    ...
    if provider == "anthropic":
        ...
        if not kwargs.get("base_url", ""):
            base_url = os.getenv("NEXA_ENDPOINT", "http://127.0.0.1:8080/v1")
        else:
            base_url = kwargs.get("base_url")
        
        model_name = kwargs.get("model_name")
        if not model_name:
            raise ValueError("❌ Nexa provider requires explicit model_name parameter. Please specify a model like 'NexaAI/Qwen3-VL-4B-MLX-8bit'")
        
        return create_nexa_llm(
            model=model_name,
            base_url=base_url,
            temperature=kwargs.get("temperature", 0.0)
        )
